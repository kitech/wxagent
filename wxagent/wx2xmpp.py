# web weixin protocol to xmpp protocol
...
class WX2Tox(QObject):
    ...
    def __init__(self, parent = None):
        ...
        self.xmpp = None  # QSleek('xxx', 'xxx')
        from .secfg import peer_xmpp_user
        self.peer_xmpp_user = peer_xmpp_user
    ...
    def initXmpp(self):
        ...
        self.xmpp = QSleek(xmpp_user, xmpp_pass, peer_xmpp_user, self)

        self.xmpp.connected.connect(self.onXmppConnected, Qt.QueuedConnection)
        self.xmpp.disconnected.connect(self.onXmppDisconnected, Qt.QueuedConnection)
        self.xmpp.groupInvite.connect(self.onXmppGroupInvite, Qt.QueuedConnection)
        self.xmpp.newGroupMessage.connect(self.onXmppGroupMessage, Qt.QueuedConnection)
        return

    def onXmppConnected(self):
        qDebug('hreee')

        if self.need_send_qrfile is True:
            from .secfg import peer_xmpp_user
            url = upload_file(self.qrpic.data())
            self.xmpp.send_message(peer_xmpp_user, 'test qrpic url....' + url)
        ...
        if len(self.wx2tox_msg_buffer) > 0:
            ...
            while len(self.wx2tox_msg_buffer) > 0:
                ...
                self.xmpp.send_message(self.peer_xmpp_user, msg)
                # ## TODO 如果发送失败，这条消息可就丢失了。
            qDebug('send buffered wx2tox msg: %s' % blen)

        return

    def onXmppDisconnected(self):
        qDebug('hreee')
        return

    def onXmppMessage(self, msg):
        qDebug('hreee')
        return

    def onXmppGroupMessage(self, group_number, message):
        qDebug('hreee:'+str(group_number))
        ...
        try:
            if peer_number == 0: pass  # it myself sent message, omit
                ...
                # mid = self.toxkit.sendMessage(self.peerToxId, message)
                self.xmpp.send_message(self.peer_xmpp_user, message)
        except Exception as ex:
            qDebug('send msg error: %s' % str(ex))

        return

    def onXmppGroupInvite(self, inv):
        qDebug('hreee')
        return
    ...
    def startWXBot(self):
        ...
        if qrpic is None:
            ...
            tkc = False
            if self.xmpp is not None:  tkc = self.xmpp.is_connected
            if tkc is True:
                url = upload_file(self.qrpic.data())
                self.xmpp.send_message(self.peer_xmpp_user, url)
            else:
    ...
    def sendMessageToTox(self, msg, fmtcc):
        ...
        fstatus = self.xmpp.is_connected
        if fstatus is True:
            try:
                # 把收到的消息发送到汇总tox端
                self.xmpp.send_message(self.peer_xmpp_user, fmtcc)
            except Exception as ex:
    ...
    def dispatchNewsappChatToTox(self, msg, fmtcc):
        ...
        if mkey in self.wxchatmap:
            ...
            number_peers = self.xmpp.muc_number_peers(groupchat.group_number)
            if number_peers < 2:
                groupchat.unsend_queue.append(fmtcc)
                # ## reinvite peer into group
                self.xmpp.muc_invite(groupchat.group_number, self.peer_xmpp_user)
            else:
                self.xmpp.muc_send_message(groupchat.group_number, fmtcc)
        else:
    ...
    def dispatchFileHelperChatToTox(self, msg, fmtcc):
        ...
        if msg.FromUserName == 'filehelper':
            mkey = msg.FromUser.FromUserName
    ...
    def createChatroom(self, msg, mkey, title):
        ...
        self.xmpp.create_muc2(group_number, title)
        self.xmpp.muc_invite(group_number, self.peer_xmpp_user)
